"""Network Utility GUI app.

Features:
1. Landing page with overview.
2. Network mapping from default gateway/ARP table.
3. Public IP lookup by IP address.
4. Queue and run .bat, .py, .bash/.sh scripts.
"""
...
class NetworkUtilityApp(tk.Tk):
    ...
    def _build_overview_tab(self) -> None:
        ...
        overview = (
            "This app helps you quickly inspect your local network and run utility scripts.\n\n"
            "• Network Mapper: identifies the current default gateway and devices discovered "
            "in the ARP table.\n"
            "• IP Lookup: fetches simple origin details for a public IP (country and org/company).\n"
            "• Script Queue: add .py, .bat/.cmd, and .bash/.sh scripts and run them one-by-one "
            "or in sequence.\n\n"
            "Tip: Run this app with admin/root permissions if your OS restricts network commands."
        )
        ttk.Label(
            self.overview_tab,
            text=overview,
    ...
    def get_arp_devices(self) -> list[DeviceRecord]:
        ...
        for line in output.splitlines():
            ...
            try:
                ...
                continue
            devices.append(DeviceRecord(ip=ip, mac=mac, note=note))

        unique: dict[str, DeviceRecord] = {d.ip: d for d in devices}
    ...
    def lookup_ip(self) -> None:
        ...
        def worker() -> None:
            try:
                ...
                self.after(0, lambda: self.lookup_output.insert(tk.END, f"Lookup error: {exc}\n"))
    ...
    def _execute_script(self, path: str) -> None:
        ...
        try:
            if ext == ".py":
                ...
                if os.name != "nt":
                    self.log_queue.put("[skip] .bat/.cmd scripts can only run on Windows.")
                    return
                cmd = ["cmd", "/c", path]
            elif ext in {".bash", ".sh"}:
                cmd = ["bash", path]
            else:
                self.log_queue.put(f"[skip] Unsupported extension: {ext}")
                return
            ...
            if result.stderr.strip():
                self.log_queue.put("[stderr] " + result.stderr.strip())
            self.log_queue.put(f"<<< Exit code: {result.returncode}\n")
        except FileNotFoundError as exc:
            self.log_queue.put(f"[error] Missing runtime for script: {exc}")
        except Exception as exc:  # noqa: BLE001
            self.log_queue.put(f"[error] Failed running script: {exc}")
